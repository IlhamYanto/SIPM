	...
	d = d.SHA3 = r.extend({
		...
		_doProcessBlock: function(a, b) {
			...
			for (f = 0; 24 > f; f++) {
				...
				for (m = 1; 25 > m; m++)
					(g = e[m]), (h = g.high), (g = g.low), (k = s[m]), 32 > k
						? (
								(l = (h << k) | (g >>> (32 - k))),
								(d = (g << k) | (h >>> (32 - k)))
							)
				...
				for (h = 0; 5 > h; h++)
					for (k = 0; 5 > k; k++)
						(m = h + 5 * k), (g = e[m]), (l = c[m]), (m =
							c[(h + 1) % 5 + 5 * k]), (d = c[(h + 2) % 5 + 5 * k]), (g.high =
							l.high ^ (~m.high & d.high)), (g.low = l.low ^ (~m.low & d.low));
				g = e[0];
				h = y[f];
				g.high ^= h.high;
				g.low ^= h.low;
